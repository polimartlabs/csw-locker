...
export const handleGetClientConfig = (address: string | undefined, searchParams: URLSearchParams) => {
...
export const handleCCS = async (address: string | undefined, contractId: string, txinfo: boolean, searchParams: URLSearchParams) => {
  let contractInfo
  try {
    ...
    const allDeployedWallets: WalletType[] = (
      await Promise.all(
        ContractTypes.map(async (wallets) => {
          const wr = await handleCCS(walletAddress, `${walletAddress}.${smartWalletName}`, true, searchParams);
